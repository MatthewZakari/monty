#ifndef MONTY_H
#define MONTY_H

#define _GNU_SOURCE
#define _POSIX_C_SOURCE 200809L

#include <stdio.h>
#include <stddef.h>
#include <stdlib.h>
#include <sys/types.h>
#include <unistd.h>
#include <fcntl.h>
#include <string.h>
#include <ctype.h>

/**
 * struct stack_s - doubly linked list representation of a stack (or queue)
 * @n: integer
 * @prev: points to the previous element of the stack (or queue)
 * @next: points to the next element of the stack (or queue)
 *
 * Description: doubly linked list node structure
 * for stack, queues, LIFO, FIFO Holberton project
 */
typedef struct stack_s
{
	int n;
	struct stack_s *prev;
	struct stack_s *next;
} stack_t;
/**
 * struct bas_s - variables -args, file, line content
 * @arg: value
 * @file: pointer to monty file
 * @content: line content
 * @lifi: flag change stack <-> queue
 * Description: carries values through the program
 */
typedef struct bas_s
{
	char *arg;
	FILE *file;
	char *content;
	int lifi;
}  bas_t;
extern bas_t bt;
/**
 * struct instruction_s - opcode and its function
 * @opcode: the opcode
 * @f: function to handle the opcode
 *
 * Description: opcode and its function
 * for stack, queues, LIFO, FIFO Holberton project
 */
typedef struct instruction_s
{
	char *opcode;
	void (*f)(stack_t **stack, unsigned int line_number);
} instruction_t;

/* function prototypes*/
char *_realloc(char *ptr, unsigned int old_size, unsigned int new_size);
ssize_t getstdin(char **lineptr, int file);
char  *clean_line(char *content);
void z_push(stack_t **top, unsigned int number);
void z_pall(stack_t **top, unsigned int number);
void z_pint(stack_t **top, unsigned int number);
int execute(char *content, stack_t **top, unsigned int counter, FILE *file);
void free_stack(stack_t *top);
void z_pop(stack_t **top, unsigned int counter);
void z_swap(stack_t **top, unsigned int counter);
void z_add(stack_t **top, unsigned int counter);
void z_nop(stack_t **top, unsigned int counter);
void z_sub(stack_t **top, unsigned int counter);
void z_div(stack_t **top, unsigned int counter);
void z_mul(stack_t **top, unsigned int counter);
void z_mod(stack_t **top, unsigned int counter);
void z_pchar(stack_t **top, unsigned int counter);
void z_pstr(stack_t **top, unsigned int counter);
void z_rotl(stack_t **top, unsigned int counter);
void z_rotr(stack_t **top, __attribute__((unused)) unsigned int counter);
void addnode(stack_t **top, int n);
void addqueue(stack_t **top, int n);
void z_queue(stack_t **top, unsigned int counter);
void z_stack(stack_t **top, unsigned int counter);

#endif

